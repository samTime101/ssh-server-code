...
export function AddQuestionPage(): React.FC { 
  // Form state
  ...
  useEffect(() => {
    if (formData.subCategoryId.length > 0) {
      const selectedSubCategory = subCategories.find(sc => 
        sc.subCategoryId === parseInt(formData.subCategoryId[0])
      );
  ...
  return (
    <><AdminPanel /><div className="min-h-screen bg-gray-50 py-8">
          <div className="max-w-4xl mx-auto px-4">
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      ...
                      {/* Sub-subcategory */}
                      <div>
                          <label htmlFor="subSubCategoryId" className="block text-sm font-medium text-gray-700 mb-1">
                              Sub-subcategory
                          </label>
                          <select
                              id="subSubCategoryId"
                      ...
                      <div className="space-y-3">
                          {formData.answers.map((answer) => (
                              <div key={answer.id} className="flex items-start space-x-3">
                                  ...
                                  {/* Answer ID */}
                                  <div className="w-8 mt-2 font-medium">
                                      {answer.id}.
                                  </div>
...
// import React, { useEffect, useState } from 'react';
// import AdminPanel from '../admin-panel';
// import { createQuestion, type QuestionFormData } from '@/services/addquestion-service';
// import { createCategory, type Category } from '@/services/category-service';
// import { getCategories, type SubCategory } from '@/services/subcategory-service';
// import type { AuthToken } from '@/types/auth';
...
//    // var categories = getSubCategories();
//     //const category = categories.find(c => c.categoryName === formData.subCategory);
//     //console.log('The category name is ' + categoryName)
//      var selectedCategory = categories.find(sc => sc.categoryId === parseInt(categoryName))
//      var subCategory = selectedCategory?.subCategories;

//      //console.log(subCategory)
//     //console.log('The sub category is ', category);
//     return subCategory || [];
//   };
...
//     const subCategory = subCategories.find(sc => sc.categoryName === formData.subCategory);
...
//     // var subCategories = categories.find(sc => sc.categoryName === value)
//     // console.log( value)
//     setFormData(prev => ({ ...prev, [name]: value }));
//   };

//   const handleAnswerTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
//     const answerType = e.target.value as 'single' | 'multiple';
//     setFormData(prev => ({ 
//       ...prev, 
//       answerType,
//       // Reset correct answers when switching types
...
//     if (formData.answers.filter(a => a.isCorrect).length === 0) {
//       alert('Please select at least one correct answer');
//       return;
//     }
...
//     // Submit the form data (in a real app, this would be an API call)
//     const tokenString = localStorage.getItem("accessToken")
//     const token: AuthToken = { access: tokenString, refresh: "" }
//     createQuestion(formData, token)
//     alert('Question added successfully!');
...
//                                   name="answerType"
//                                   value="single"
//                                   checked={formData.answerType === 'single'}
...
//                                   name="answerType"
//                                   value="multiple"
//                                   checked={formData.answerType === 'multiple'}
//                                   onChange={handleAnswerTypeChange}
//                                   className="text-blue-600 focus:ring-blue-500" />
//                               <span className="ml-2">Multiple Correct Answers</span>
//                           </label>
